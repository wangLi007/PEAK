...
export default class {
  ...
  async purchase(amount, decimal = 18) {
    return new Promise((resolve, reject) => {
      this.contractObj
        .purchase(ethers.utils.parseUnits(amount, decimal))
        .then(async res => {
          Toast.loading({
            message: $t('msg.28'),
            forbidClick: true,
  ...
  async viewInfo(pid) {
    return new Promise((resolve, reject) => {
      this.contractObj
        .viewInfo(+pid, this.defaultAccount)
        .then(async res => {
  ...
  stake(pid, amount, power, referralAddr = '0x0000000000000000000000000000000000000000') {
    ...
    return new Promise((resolve, reject) => {
      this.contractObj
        .stake(+pid, ethers.utils.parseUnits(amount, power), referralAddr)
        .then(async res => {
          await res?.wait();
          resolve(true);
        })
  ...
  addPool(obj) {
    ...
    const times = [
      obj.startTime,
      obj.endTime,
      isRelease ? obj.releaseStartTime : 0,
      isRelease ? obj.releaseEndTime : 0,
    ];
    ...
    return new Promise((resolve, reject) => {
      this.contractObj
        .addPool(times, tokens, maxOut, invRate)
        .then(async res => {
          await res?.wait();
          resolve(true);
        })
        ...
        .then(res => {
          ...
          const list = tokens.map((item, index) => {
            return {
              ...
              progress:
                +infos[index][0] === 0 ? 0 : bpMul(bpDiv(infos[index][0], infos[index][1]), 100),
            };
          });
  ...
  getTigerDaoInfo(address) {
    return new Promise((resolve, reject) => {
      this.contractObj
        .getTigerDaoInfo(address, useAppStore().defaultAccount)
        .then(async res => {
          console.log(res, 'rawData');
  ...
  createTigerDao(obj) {
    ...
    return new Promise((resolve, reject) => {
      this.contractObj
        .createTigerDao(quotas, tokens, times)
        .then(async res => {
          await res?.wait();
          resolve(+res);
        })
